...
// Settings are in C:\Users\MenMe\Downloads\um2229-teseoliv3-gnss-module--software-manual-stmicroelectronics_but_like_real_this_time.pdf 12.14 CDB-ID 190 - CDB-ID 201 - CDB-ID 228 - NMEA on UART message list parameters
void gnssConf_set() {
    const uint32_t NMEA_PORT_MSG_LIST_0_LOW_VAL = 0;
...
void gnssConf_printBuffer() {
    ...
    const int avail = Serial_GNSS.available();
    if(avail) {
        Serial.printf("The thing says: \"", avail);
        for(uint32_t idx = 0; idx < avail; idx++) {
            const int readByte = Serial_GNSS.read();
            Serial.printf("%c", readByte);
        }
...
void setup() {
    ...
    Serial.printf(
   "GNSS thing MNEA messages configuration setter\n"
